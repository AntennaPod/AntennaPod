...
public class FeedMedia extends FeedFile implements Playable {
	...
	public FeedMedia(long id, FeedItem item, int duration, int position, int played_duration,
	...
	public void setPosition(int position) {
		if (position > this.position)
			played_duration += (position - this.position);
	...
	public void saveCurrentPosition(SharedPreferences pref, int newPosition) {
	    ...
	    if (item.getPaymentLink() != null && item.getFlattrStatus().getUnflattred() && (played_duration > 0.8*duration)) {
	    	...
	    	new FlattrClickWorker(PodcastApp.getInstance()).executeAsync();
	    }
